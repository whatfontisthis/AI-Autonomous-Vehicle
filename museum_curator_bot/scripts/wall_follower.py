...
def calculate_lasers_range(data):
    ...
    laser_sensors["n"] = np.mean(
        np.setdiff1d(data.ranges[:35] + data.ranges[-35:], [0])
    )
...
def laser_callback(data):
    ...
    if orbit == 0:
        ...
        if laser_sensors["n"] > wall_distance_forward and (
            laser_sensors["w"] > wall_distance_side
        ...
        if (
            ...
            laser_sensors["nw"] <= wall_distance_diagonal
            or laser_sensors["ne"] <= wall_distance_diagonal
        ):
            turn_right = True
        else:
            if (
                laser_sensors["ne"] < wall_distance_diagonal
